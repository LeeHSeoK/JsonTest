...
function notice(keyword) {

    $.ajax({
        type: "GET",
        url: `https://apis.data.go.kr/B551011/KorService1/searchKeyword1?serviceKey=tussmhxuu1xAlRv9cJ9NQP8LtOwmZjojrJeUaGLT6sI0jd91wXKCsuR4Yak8I08mDILJw17m2HRvvjXJNYzNnQ%3D%3D&numOfRows=10&pageNo=1&MobileOS=ETC&MobileApp=AppTest&_type=json&listYN=Y&arrange=A&keyword=${keyword}&contentTypeId=15`,
        data: {},
        success: function(response) {
            let items = response.response.body.items.item;
            console.log(items)
            ...
            for (let j = 0; j < items.length; j++) {
                if (items[j].firstimage && items[j].firstimage.trim() !== "") {
                    title.push(`<li><a href="https://search.naver.com/search.naver?sm=mtp_hty.top&where=m&query=${items[j].title}">${items[j].title}</a></li>`);
                    if(img.length <1){
                        img.push(`<img src="${items[j].firstimage}" th:unless="null" alt="Image" class="square-image">`);
                    }
            ...
            // 이미지와 제목을 화면에 출력
            $('.notice-list').empty().append(title.join(''))
        },
        error: function(xhr, status, error) {
            console.error('Error fetching data:', error);
        }
